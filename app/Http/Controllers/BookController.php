...
class BookController extends Controller
{
    ...
    public function book_create()
    {
        ...
        if ($userType === 'employee') {
            $orgIds = EmployeeOrganizationEmployee::where('employee_id', $authUser->id)->get()->pluck('organization_id')->toArray();
            array_push($orgIds, $authUser->organization_id);
        }
        if ($userType === 'user') {
            $orgIds = UserOrganizationMapping::where('user_id', $authUser->id)->get()->pluck('organization_id')->toArray();
            array_push($orgIds, $authUser?->organization_id);
        }
                        ...
                        foreach ($request->param_ids as $orgServiceParamKey => $orgServiceParamId) 
                        {
                            ...
                            if ($request->param_names[$orgServiceParamKey] === ServiceParametersHelper::REFERENCE_FROM_SERIES_PARAM) 
                            {
                                ...
                                foreach ($paramValues as $paramValue) {
                                    $exists = OrganizationBookParameter::where('parameter_name', ServiceParametersHelper::REFERENCE_FROM_SERIES_PARAM)
    ...
    public function edit_book($id)
    {
        ...
        if (isset($book)) {
            ...
            foreach ($book -> levels as $approvalLevel => &$approver) {
                $levelEmployees = Employee::where('organization_id', $approver -> organization_id) -> get();
                $serviceAlias = $book ?-> service ?-> alias;
                $modelName = isset(ConstantHelper::SERVICE_ALIAS_MODELS[$serviceAlias]) ? ConstantHelper::SERVICE_ALIAS_MODELS[$serviceAlias] : '';
                if (isset($modelName)) {
                    $model = resolve('App\\Models\\' . $modelName);
            ...
            foreach ($book -> amendments as &$amendment) {
                $amendEmployees = Employee::where('organization_id', $amendment -> organization_id) -> get();
            ...
            if ($request->params && $request->param_ids && $request->param_names) {
                foreach ($request->param_ids as $bookParamKey => $bookParamId) {
                    if ($request->param_names[$bookParamKey] === ServiceParametersHelper::REFERENCE_FROM_SERIES_PARAM) {
                        ...
                        foreach ($paramValues as $paramValue) {
                            $exists = OrganizationBookParameter::where('book_id', '!=', $update->id)->where('parameter_name', ServiceParametersHelper::REFERENCE_FROM_SERIES_PARAM)
                                ->whereJsonContains('parameter_value', (string) $paramValue)->first();
                            if ($exists) {
                                $series = Book::find($paramValue);
                                $seriesName = isset($series) ? $series->book_code : 'A Book Code';
    ...
    public function getEmployeesForApprovalOrgWise(Request $request)
    {
        try {
            $employees = Employee::where(function ($query) use ($request) {
                $query->whereHas('access_rights_org', function ($subQuery) use ($request) {
                    $subQuery->where('organization_id', $request->organization_id);
