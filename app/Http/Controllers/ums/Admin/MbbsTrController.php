...
namespace App\Http\ums\Controllers\Admin;
...
class MbbsTrController extends  AdminController
{
    ...
    public function index(Request $request)
    {
    	...
    	$batch = $request->batch;
    	$batchPrefix = batchPrefixByBatch($request->batch);
		...
		if($request->form_type=='compartment'){
			$mbbs_exam_forms = ExamFee::withTrashed()->where('course_id',$request->course)
		...
		$students->each(function ($item_student, $key_student) use ($course_id,$semester_id,$session,$request,$batch,$batchPrefix){
			...
			$item_student['subjects_group_all'] = $subjects_group_all->each(function ($item_group, $key_group) use ($course_id,$semester_id,$item_student,$session,$request,$batch,$batchPrefix){
				...
				$grace_mark = $item_student->final_result_grade_theory_practical($item_student->roll_no,$course_id,$semester_id,$item_group->combined_subject_name);
				$grace_mark_practical = $item_student->final_result_grade_practical($item_student->roll_no,$course_id,$semester_id,$item_group->combined_subject_name);
				$grace_mark_theory = $item_student->final_result_grade_theory($item_student->roll_no,$course_id,$semester_id,$item_group->combined_subject_name);
				$item_group['grace_mark'] = $grace_mark;
				$item_group['grace_mark_practical'] = $grace_mark_practical;
				$item_group['grace_mark_theory'] = $grace_mark_theory;
			});
		...
		return view('admin.tr.mbbs-tr-first', $data);
    }
    ...
    public function index_2019_2020And2020_2021(Request $request)
    {
		...
		return view('admin.tr.index_2019_2020And2020_2021', $data);
    }
    ...
    public function dpharma_tr(Request $request)
    {
        ...
        if($request->pdf_download!=null){
			$data['download'] = 'pdf';
            $htmlfile = view('admin.tr.dpharma_tr', $data)->render();
			$pdf = app()->make('dompdf.wrapper');
			$pdf->loadHTML($htmlfile,'UTF-8')
			->setWarnings(false)
			->setPaper('a1', 'landscape');
		...
		return view('admin.tr.dpharma_tr', $data);
    }
	...
	public function mbbs_allowed_students(Request $request)
    {
		...
		->distinct()
		->get();
		return view('admin.mbbs.mbbs_allowed_students', $data);
    }
