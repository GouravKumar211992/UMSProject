...
public function batchArray()
{
    ...
    public function index_2019_2020And2020_2021(Request $request)
    {
        $data['courses'] = Course::where('id',49)->get();
    	$data['semesters'] = Semester::where('course_id',49)->orderBy('id','asc')->get();
    	$data['selected_semester'] = Semester::find($request->semester);
    	$data['selected_session'] = AcademicSession::get();
		 $data['selected_batch'] = $this->batchArray();
		 $data['selected_form_type'] = $request->form_type;
    	$session = $request->session;
        $course_id = $request->course;
    	$semester_id = $request->semester;
    	$batch = $request->batch;
