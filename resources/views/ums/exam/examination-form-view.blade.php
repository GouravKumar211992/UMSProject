...
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEJx3l3b4L03lJfXJm0FE5eazl1m6UvcN5jfbp5M/TpbdzFf8au0kbzz13F4" crossorigin="anonymous">
...
@if($form_data->is_agree==1)
	<button type="button" onclick="print()" class="btn btn-warning "  style="padding: 10px 20px; background:success; border-radius:8px;">
		<i class="fa fa-print" aria-hidden="true"></i> Print Application
	</button>
    <a  href="{{url('/exam/pay-success')}}?id={{$form_data->id}}" class="btn btn-sm "  style="padding: 10px 20px; background:orange; border-radius:8px;">
        <i class="fa fa-print" aria-hidden="true"></i> Payment Slip
    </a>
    <a href="{{url('/exam-form')}}" class="btn btn-warning "  style="padding: 10px 20px; background:orange; border-radius:8px;">
        <i class="" aria-hidden="true"></i> Go To Dashboard
    </a>
	@endif
	@php $admitcard= $form_data->admitcard; @endphp
	@if($admitcard)
	<a type="button" href="{{route('download-admit-card',['id'=>$form_data->id])}}"  class="btn btn-warning " style="padding: 10px 20px; background:orange; border-radius:8px;">
        ...
        @if($form_data->scribe=='yes' || $form_data->scribe=='Yes')
            @if($admitcard->scribeDetails)
                <a type="button" href="{{route('scribe-admitcard',['id'=>$form_data->id])}}"  class="btn btn-warning " style="padding: 10px 20px; background:orange; border-radius:8px;">
                    <i class="fa fa-download" aria-hidden="true"></i>Download Scribe Admit Card
                </a>
            @else
                <a type="button" href="{{route('scribe-form',$form_data->id)}}"  class="btn btn-warning "  style="padding: 10px 20px; background:orange; border-radius:8px;">
...
@endphp
    ...
    <div style="border: black thin solid; width:700px; padding: 10px; font-family:Arial;">
        ...
        <p style="text-align: center; margin-bottom: 5px;"><img src="{{asset('img\cerlogo.png')}}"></p>
...
<script>
    $(document).ready(function() {
      ...
      <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js" integrity="sha384-pzjw8f+ua7Kw1TIq0v8FqPr7Wdy0g6tFlujS9Bq8Z3bFg9xlwli0jvQmPpjgi0Lg" crossorigin="anonymous"></script>
